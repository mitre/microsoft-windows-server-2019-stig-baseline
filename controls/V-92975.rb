...
control 'V-92975' do
  ...
  If temporary accounts are used, the operating system must be configured to automatically terminate these types of accounts after a #{input('org_name')[:acronym]}-defined time period of #{input('temporary_account_period')} hours.
  ...
  If \"AccountExpirationDate\" has not been defined within #{input('temporary_account_period')} hours for any temporary user account, this is a finding.

  Member servers and standalone systems:
  Open \"Command Prompt\".
  Run \"Net user [username]\", where [username] is the name of the temporary user account.
  If \"Account expires\" has not been defined within #{input('temporary_account_period')} hours for any temporary user account, this is a finding."
  desc 'fix', "Configure temporary user accounts to automatically expire within #{input('temporary_account_period')} hours.
