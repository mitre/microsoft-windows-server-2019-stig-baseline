...
control 'SV-205807' do
  title "Windows Server 2019 must employ a deny-all, permit-by-exception policy
...
is commonly referred to as whitelisting.
  ...
  desc  'check', "
    ...
    Verify the operating system employs a deny-all, permit-by-exception policy
to allow the execution of authorized software programs.

    If an application whitelisting program is not in use on the system, this is
a finding.
...
AppLocker:
    ...
    If the AppLocker PowerShell module has not been imported previously,
...
location and file name appropriate for the system:
    ...
    This will produce an xml file with the effective settings that can be
viewed in a browser or opened in a program such as Excel for review.
...
programs.
    ...
    Configuration of whitelisting applications will vary by the program.
AppLocker is a whitelisting application built into Windows Server.
