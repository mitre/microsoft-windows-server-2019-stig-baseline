control 'SV-205847' do
  title 'Windows Server 2019 manually managed application account passwords must be changed at least every 365 days or when a system administrator with knowledge of the password leaves the organization.'
  ...
  desc 'check', %q(Determine if manually managed application/service accounts exist. If none exist, this is NA.
    ...
    If the "Password Last Set" date is more than 365 days old, this is a finding.)
  desc 'fix', 'Change passwords for manually managed application/service accounts at least every 365 days or when an administrator with knowledge of the password leaves the organization.
