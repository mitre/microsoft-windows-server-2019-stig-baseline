...
control "V-93381" do
  title "Windows Server 2019 must have the roles and features required by the
system documented."
...
Core) further reduces this when selected at installation."
  desc  "rationale", ""
  desc  "check", "
    Required roles and features will vary based on the function of the
individual system.

    Roles and features specifically required to be disabled per the STIG are
identified in separate requirements.

    If the organization has not documented the roles and features required for
the system(s), this is a finding.

    The PowerShell command \"Get-WindowsFeature\" will list all roles and
features with an \"Install State\".
  "
