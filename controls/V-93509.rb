...
control 'V-93509' do
  title "Windows Server 2019 directory service must be configured to terminate LDAP-based network connections to the directory server after #{input('maximum_idle_time')} minutes of inactivity."
  ...
  desc  'check', "This applies to domain controllers. It is NA for other systems.
    ...
    If the value for MaxConnIdleTime is greater than \"#{input('maximum_idle_time')*60}\" (#{input('maximum_idle_time')} minutes) or is not specified, this is a finding.
    ...
    If the results do not specify a \"MaxConnIdleTime\" or it has a value greater than \"#{input('maximum_idle_time')*60}\" (#{input('maximum_idle_time')} minutes), this is a finding."
  desc  'fix', "Configure the directory service to terminate LDAP-based network connections to the directory server after #{input('maximum_idle_time')} minutes of inactivity.
    Open an elevated \"Command prompt\" (run as administrator).
    Enter \"ntdsutil\".
    At the \"ntdsutil:\" prompt, enter \"LDAP policies\".
    At the \"ldap policy:\" prompt, enter \"connections\".
