...
control "V-93271" do
  title "Windows Server 2019 directory data (outside the root DSE) of a
non-public directory must be configured to prevent anonymous access."
...
naming context and this is a finding.
    ...
    The following network controls allow the finding severity to be downgraded
to a CAT II since these measures lower the risk associated with anonymous
access.
...
3268, and 3269 from client hosts not explicitly identified by domain (.mil) or
IP address.
  "
  desc  "fix", "
...
access.
    ...
    Changing the access permissions on the domain naming context object (from
the secure defaults) could enable anonymous access. If the check procedures
...
Edit console (adsiedit.msc).
    ...
    The dsHeuristics option is used. This is addressed in check V-8555 in the
AD Forest STIG.
  "
  impact 0.7
  tag severity: nil
