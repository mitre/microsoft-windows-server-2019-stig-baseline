...
control 'SV-205695' do
  title "Windows Server 2019 domain controllers must run on a machine dedicated
to that function."
...
interfere with intrusion detection or prevention services.
  ...
  desc  'check', "
    ...
    Select \"AD DS\" in the left pane and the server name under \"Servers\" to
the right.

    Select \"Add (or Remove) Roles and Features\" from \"Tasks\" in the \"Roles
and Features\" section. (Cancel before any changes are made.)
...
controller setup will include the following:
    ...
    If any roles not requiring installation on a domain controller are
installed, this is a finding.

    A Domain Name System (DNS) server integrated with the directory server
(e.g., AD-integrated DNS) is an acceptable application. However, the DNS server
must comply with the DNS STIG security requirements.
