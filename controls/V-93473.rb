...
control "V-93473" do
  title "Windows Server 2019 passwords for the built-in Administrator account
...
by default.
  ...
  desc  "check", "
    ...
    Enter \"Get-ADUser -Filter * -Properties SID, PasswordLastSet | Where SID
-Like \"*-500\" | Ft Name, SID, PasswordLastSet\".
...
finding.
    ...
    Enter 'Net User [account name] | Find /i \"Password Last Set\"', where
[account name] is the name of the built-in administrator account.
...
member servers to accomplish this.
  "
  impact 0.5
  tag severity: nil
