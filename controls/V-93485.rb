...
control "V-93485" do
  title "Windows Server 2019 PKI certificates associated with user accounts
must be issued by a DoD PKI or an approved External Certificate Authority
(ECA)."
...
authentication functions."
  ...
  desc  "check", "
    ...
    Exclude disabled accounts (e.g., DefaultAccount, Guest) and the krbtgt
account.
...
is a finding.
    ...
    For standard NIPRNet certificates, the individual's identifier is in the
format of an Electronic Data Interchange - Personnel Identifier (EDI-PI).
...
Component's CIO, this is a CAT II finding.
  "
  desc  "fix", "Map user accounts to PKI certificates using the appropriate
User Principal Name (UPN) for the network. See PKE documentation for details."
  impact 0.7
  tag severity: nil
