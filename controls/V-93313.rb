...
control "V-93313" do
  title "Windows Server 2019 Exploit Protection system-level mitigation, Data
Execution Prevention (DEP), must be on."
  desc  "Exploit protection enables mitigations against potential threats at
the system and application level.  Several mitigations, including \"Data
Execution Prevention (DEP)\", are enabled by default at the system level. DEP
prevents code from being run from data-only memory pages. If this is turned
...
\"NOTSET\".
  ...
  "
  desc  "fix", "
    Ensure Exploit Protection system-level mitigation, \"Data Execution
...
the following to the XML file will explicitly turn DEP on (other system level
