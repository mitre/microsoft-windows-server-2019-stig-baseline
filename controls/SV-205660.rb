control 'SV-205660' do
  ...
  desc %q(A system is more vulnerable to unauthorized access when system users recycle the same password several times without being required to change to a unique password on a regularly scheduled basis. This enables users to effectively negate the purpose of mandating periodic password changes. The default value is "#{input('password_history_size')}" for Windows domain systems. #{input('org_name')[:acronym]} has decided this is the appropriate value for all Windows systems.)
  desc 'check', %q(Verify the effective setting in Local Group Policy Editor.
    Run "gpedit.msc".
  ...
  desc 'fix', %q(Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Account Policies >> Password Policy >> "Enforce password history" to "#{input('password_history_size')}" passwords remembered.)
  impact 0.5
  tag severity: nil
