...
control "V-93149" do
  ...
  desc  'check', "If the following registry value does not exist or is not configured as specified, this is a finding:
    ...
    \"#{input('org_name')[:acronym]} Notice and Consent Banner\", \"US #{input('org_name')[:full_form]} Warning Statement\", or an organization-defined equivalent.
  ...
  desc  'fix', "Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> Security Options >> \"Interactive Logon: Message title for users attempting to log on\" to \"#{input('org_name')[:acronym]} Notice and Consent Banner\", \"US #{input('org_name')[:full_form]} Warning Statement\", or an organization-defined equivalent.
