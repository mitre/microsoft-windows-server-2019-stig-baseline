...
control "V-93483" do
  title "Windows Server 2019 domain Controller PKI certificates must be issued by the #{input('org_name')} PKI or an approved External Certificate Authority (ECA)."
  ...
  desc  "check", "This applies to domain controllers. It is NA for other systems.
    ...
    If the \"Issued By\" field of the PKI certificate being used by the domain controller does not indicate the issuing CA is part of the #{input('org_name')} PKI or an approved ECA, this is a finding.
    If the certificates in use are issued by a CA authorized by the Component's CIO, this is a CAT II finding.
    There are multiple sources from which lists of valid #{input('org_name')} CAs and approved ECAs can be obtained:

    The Global Directory Service (GDS) website provides an online source. The address for this site is https://crl.gds.disa.mil.

    #{input('org_name')} Public Key Enablement (PKE) Engineering Support maintains the InstallRoot utility to manage #{input('org_name')} supported root certificates on Windows computers, which includes a list of authorized CAs. The utility package can be downloaded from the PKI and PKE Tools page on IASE:
    http://iase.disa.mil/pki-pke/function_pages/tools.html"
  desc  "fix", "Obtain a server certificate for the domain controller issued by the #{input('org_name')} PKI or an approved ECA."
  impact 0.7
  tag 'severity': nil
