...
control "V-92977" do
  title "Windows Server 2019 must automatically remove or disable emergency accounts after the crisis is resolved or within #{input('emergency_account_period')} hours."
  ...
  desc  'check', "Determine if emergency administrator accounts are used and identify any that exist. If none exist, this is NA.
    ...
    If \"AccountExpirationDate\" has been defined and is not within #{input('emergency_account_period')} hours for an emergency administrator account, this is a finding.

    Member servers and standalone systems:
    Open \"Command Prompt\".
    Run \"Net user [username]\", where [username] is the name of the emergency account.
    If \"Account expires\" has been defined and is not within #{input('emergency_account_period')} hours for an emergency administrator account, this is a finding."
  desc  'fix', "Remove emergency administrator accounts after a crisis has been resolved or configure the accounts to automatically expire within #{input('emergency_account_period')} hours.
      ...
      emergency_accounts.each do |emergency_account|
        ...
        if emergency_account.fetch("WhenCreated") == nil
          ...
          describe "Account expiration set for #{account_name}" do
            ...
            it { should cmp <= input('emergency_account_period')/24}
          end
        end
      end
    end
