...
control "V-93341" do
  ...
  #   end
    ...
    describe 'DEP is required to be enabled on Lynx' do
      subject { powershell(dep_script).strip }
      it { should_not eq '2' }
    end
    describe 'ASLR Force Relocate Image is required to be enabled on Lynx' do
      subject { powershell(aslr_forcerelocimage_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Export Address Filter is required to be enabled on Lynx' do
      subject { powershell(payload_enexpaddrfil_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Export Address Filter Plus is required to be enabled on Lynx' do
      subject { powershell(payload_enexpaddrfilplus_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Import Address Filter is required to be enabled on Lynx' do
      subject { powershell(payload_enimpaddrfil_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Rop Stack Pivot is required to be enabled on Lynx' do
      subject { powershell(payload_enropstacpiv_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Rop Caller Check is required to be enabled on Lynx' do
      subject { powershell(payload_enropcalleche_script).strip }
      it { should_not eq '2' }
    end
    describe 'Payload Enable Rop Sim Exec is required to be enabled on Lynx' do
