...
control "V-93491" do
  title "Windows Server 2019 must have the US #{input('org_name')} CCEB Interoperability Root CA cross-certificates in the Untrusted Certificates Store on unclassified systems."
  desc  "To ensure users do not experience denial of service when performing certificate-based authentication to #{input('org_name')} websites due to the system chaining to a root other than #{input('org_name')} Root CAs, the US #{input('org_name')} CCEB Interoperability Root CA cross-certificates must be installed in the Untrusted Certificate Store. This requirement only applies to unclassified systems."
  desc  "rationale", ""
