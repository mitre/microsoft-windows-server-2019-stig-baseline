...
control 'SV-236001' do
  title "The Windows Explorer Preview pane must be disabled for Windows Server
2019."
  desc  "A known vulnerability in Windows could allow the execution of
malicious code by either opening a compromised document or viewing it in the
...
\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer
  ...
  "
  desc  'fix', "
    Ensure the following settings are configured for Windows Server 2019
locally or applied through group policy.

    Configure the policy value for User Configuration >> Administrative
Templates >> Windows Components >> File Explorer >> Explorer Frame Pane \"Turn
off Preview Pane\" to \"Enabled\".

    Configure the policy value for User Configuration >> Administrative
Templates >> Windows Components >> File Explorer >> Explorer Frame Pane \"Turn
on or off details pane\" to \"Enabled\" and \"Configure details pane\" to
\"Always hide\".
  "
  impact 0.5
  tag severity: 'medium'
