control 'SV-205632' do
  ...
  desc 'fix', %q(Configure the policy value for Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> Security Options >> "Interactive Logon: Message title for users attempting to log on" to "#{input('LegalNoticeCaption').join('", "')}", or an organization-defined equivalent.
